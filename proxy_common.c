...
/**
 * Sends a PROXY_KEY message to each public multicast address.
 */
void send_proxy_key()
{
    ...
    for (i = 0; i < pub_multi_count; i++) {
        if (nb_sendto(listener, packet, meslen, 0, 
                      (struct sockaddr *)&pub_multi[i],
                      family_len(pub_multi[i])) == SOCKET_ERROR) {
            sockerror(0, 0, 0, "Error sending PROXY_KEY");
        } else {
            if ((rval = getnameinfo((struct sockaddr *)&pub_multi[i],
                    family_len(pub_multi[i]), pubname, sizeof(pubname), NULL, 0,
                    NI_NUMERICHOST)) != 0) {
                log1(0, 0, 0, "getnameinfo failed: %s", gai_strerror(rval));
            }
