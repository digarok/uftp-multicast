                    ...
                    unsigned meslen, uint16_t txseq)
{
    ...
    if (group->fileinfo.naklist[seq]) {
        ...
        if ((group->fileinfo.cache_end == group->fileinfo.blocks - 1) &&
                (group->fileinfo.size % group->blocksize != 0)) {
            group->fileinfo.cache_len = ((group->fileinfo.cache_end -
                    group->fileinfo.cache_start) * group->blocksize) + 
                    (group->fileinfo.size % group->blocksize);
        } else {
            group->fileinfo.cache_len = ((group->fileinfo.cache_end -
